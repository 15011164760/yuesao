(function () {
    var placeOrder = JSON.parse(localStorage.getItem('placeOrder') || '{}')
    	...
    	function cPlugin(o){
    		var sjObj=o;
		        ...
		        sjObj.defaults={
		            ...
		            df_box:function(){return $("<div class='df-box "+(sjObj.opt.alwaysShow?"alwaysShow":"")+"' style='line-height:50px;'></div>")},
		            df_main:function(){return $("<div class='df-main'>")},
		            df_btn:function(){
		                if(sjObj.opt.alwaysShow){
		                    return
		                }
		                return $("<div class='df-btn' style='height:50px'><div class='df-ok'>"+sjObj.opt.okText+"</div><div class='df-no'>"+sjObj.opt.cancelText+"</div></div>")
		            },
		            ...
		            getYear:function(){
		                ...
		                if (this.startYear&&this.endYear) {
		                	for (var i = this.startYear; i <= this.endYear; i++) {
		                		sjObj.opt.yyArr.push(i);
		                	}
		                    sjObj.opt.getArr(this.nowTime)
		                }
		            },
		            //核心内容居中
		            setCenter:function(){
		            ...
		            buildHTml:function(){
		                ...
		                if(sjObj.opt.alwaysShow){
		                        ...
		                        sjObj.opt.timeElm.prepend(sjObj.opt.t_box.append(main.append(wrap)));
		                }else{
		                ...
		                if(sjObj.opt.showNowTime){
		                    ...
		                    if(sjObj.opt.Seconds)sjObj.opt.timeElm.find("[data-class='s'] .df-li").each(function(){
		                        if(parseInt($(this).attr("data-val"))==parseInt(Seconds)){
		                            var pY=-($(this).index()-2)*sjObj.opt.height;
		                            $(this).parent().css({"transform":"translate(0,65px)"})
		                        }
		                    })
		                }else{
		                	if(sjObj.opt.Year)sjObj.opt.timeElm.find("[data-class='yyyy'] .df-li").each(function(){
		            ...
		            bindFun:function(){
		                //关闭事件
		                sjObj.opt.timeElm.find(".df-no").on("touchend",function(){
		                    $(this).parent().parent().parent().remove();
		                    $("html").removeClass("ov_hi");
		                    sjObj.opt.onfun();
		                })
		                sjObj.opt.timeElm.find(".df-ok").on("touchend",function(){
		                    var str="";
		                    if(sjObj.opt.Year) {
		            ...
		            moveElm:function(eml){
		                return $(eml).each(function(){
		                    ...
		                    $(this).unbind("touchstart").on("touchstart",moveStart)//触摸起始//鼠标按下
